  ...
  <main class="ax-ctrl-content">
    ...
    <div class="ax-ctrl-pane" id="ax-pane-live-tv">
      ...
      <div style="background:rgba(255,45,85,0.08);border:1px solid rgba(255,45,85,0.25);border-radius:8px;padding:12px 16px;margin-bottom:16px;font-size:12px;line-height:1.7;color:var(--text-dim);">
        ...
        This is AURENIX's primary 24/7 live television channel.
      </div>
...
async function _seedInitialChannels() {
  try {
    ...
    const seeds = [
      ...
      { id: 'A2',  name: 'AURENIX MUSIC',      label: 'MUSIC',      color: '#b8860b', channel_type: 'music',  mode: 'ordered',  sort_order: 1, description: 'Music-only channel with Founder-curated tracks.',             enabled: true, loop: true },
      { id: 'A3',  name: 'AURENIX VIDEO',      label: 'VIDEO',      color: '#8b00ff', channel_type: 'video',  mode: 'ordered',  sort_order: 2, description: 'Video broadcast channel — films, shows, music videos.',        enabled: true, loop: true },
      { id: 'A4',  name: 'AURENIX FUNNY',      label: 'FUNNY',      color: '#ff9500', channel_type: 'mixed',  mode: 'shuffle',  sort_order: 3, description: 'Comedy and funny clip channel — approved clips only.',         enabled: true, loop: true },
      { id: 'A5',  name: 'AURENIX AFTER DARK', label: 'AFTER DARK', color: '#9b59b6', channel_type: 'mixed',  mode: 'ordered',  sort_order: 4, description: 'Nighttime independent programming — its own media pool.',      enabled: true, loop: true },
    ];
